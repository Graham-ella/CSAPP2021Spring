...
{
    if (!strcmp(argv[0], "quit")) {//如果是quit指令
        exit(0);
    }
    else if (!strcmp(argv[0], "jobs")) {//如果为jobs，用listjobs输出作业列表的所有作业信息
        listjobs(jobs);
        return 1;
    }
    else if (!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")) {//如果为bg或fg，调用do_bgfg
        do_bgfg(argv);
        return 1;
    }
...
void sigchld_handler(int sig) 
{
    ...
    struct job_t* job = NULL;//新建job_t的结构体指针job
    //struct job_t {              /* The job struct */
    //    pid_t pid;              /* job PID */
    //    int jid;                /* job ID [1, 2, ...] */
    //    int state;              /* UNDEF, BG, FG, or ST */
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
    {
        if (WIFEXITED(status)) {//如果子进程是正常终止的
            ...
            deletejob(jobs, pid);/* deletejob - Delete a job whose PID=pid from the job list */
            sigprocmask(SIG_SETMASK, &prev_mask, NULL);//该进程新的信号屏蔽字将被pre_mask的信号集的值代替
        }
        else if (WIFSIGNALED(status)) {//通过一个未被捕获的信号终止
            job = getjobpid(jobs, pid);/* getjobpid  - Find a job (by PID) on the job list */
            sigprocmask(SIG_BLOCK, &mask, &prev_mask);//阻塞所有信号
...
void sigint_handler(int sig) 
{
    ...
    sigprocmask(SIG_BLOCK, &mask, &prev_mask);  //阻塞所有信号
    pid = fgpid(jobs);/* fgpid - Return PID of current foreground job, 0 if no such job */
    sigprocmask(SIG_SETMASK, &prev_mask, NULL);
